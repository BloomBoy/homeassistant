...
from zigpy.zcl.clusters.general import (
    Basic, Groups, Identify, LevelControl, OnOff, Ota, PowerConfiguration,
)
from zhaquirks.const import (
    BUTTON, COMMAND, DEVICE_TYPE, DOUBLE_PRESS, ENDPOINTS, INPUT_CLUSTERS,
    LONG_PRESS, LONG_RELEASE, MODELS_INFO, OUTPUT_CLUSTERS, PRESS_TYPE,
    PROFILE_ID, QUADRUPLE_PRESS, QUINTUPLE_PRESS, RIGHT, SHORT_PRESS,
    SHORT_RELEASE, TRIPLE_PRESS, TURN_ON, ZHA_SEND_EVENT,
)
...
class PhilipsRemoteCluster(CustomCluster):
    ...
    def handle_cluster_request(
        self, hdr: foundation.ZCLHeader, args: list, *, dst_addressing=None
    ):
        """Handle the cluster command."""
        _LOGGER.debug(
            "PhilipsRemoteCluster - handle_cluster_request tsn: [%s] command id: %s - args: [%s]",
            hdr.tsn, hdr.command_id, args,
        )
